...
async function batchEmbed(texts: string[]): Promise<number[][]> {
  ...
  // OpenAI supports up to 2048 inputs per call — use larger batches
  const r = await fetch('https://api.openai.com/v1/embeddings', {
    method: 'POST',
    headers: { Authorization: `Bearer ${OPENAI_KEY}`, 'Content-Type': 'application/json' },
...
async function main() {
  ...
  for (const row of result.rows) {
    ...
    try {
      // 1. Node-level embedding (title + description)
      ...
      // Embed in batches of 50 (OpenAI supports up to 2048, but keep payload reasonable)
      ...
      // Batch insert all chunks in one db.batch() call
      ...
      // libsql batch supports up to ~100 statements — split if needed
      ...
      failed++;
      console.error(`  ✗ node ${nodeId}: ${err.message}`);
      // Mark back as not_chunked so it can be retried
